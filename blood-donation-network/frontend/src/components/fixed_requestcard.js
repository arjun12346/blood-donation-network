...
import {
  MapPin, Phone, Building2, Droplets, Clock, Shield, AlertTriangle,
  CheckCircle2, Flag, User
} from 'lucide-react'
...
const BLOOD_GROUP_COLORS = {
  'A+': 'bg-rose-500', 'A-': 'bg-rose-600',
  'B+': 'bg-sky-500', 'B-': 'bg-sky-600',
  'AB+': 'bg-violet-500', 'AB-': 'bg-violet-600',
      ...
      <div className="p-5">
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex items-start gap-3 flex-1">
            <div className={\`\${bloodColor} text-white font-bold text-base w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0\`}>
              {request.bloodGroup}
            </div>
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 mb-3">
          ...
          <div className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400">
            ...
            <span className="truncate">{request.nearestHospital?.name} <span className="text-emerald-600 font-medium">{request.nearestHospital?.distance}</span></span>
          </div>
          <div className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400">
        ...
        <div className="bg-white/70 dark:bg-gray-800/40 rounded-xl p-3 mb-3">
          ...
          <span className="text-[11px] bg-white dark:bg-gray-900 rounded-lg px-2 py-1 border dark:border-gray-700 text-gray-500 dark:text-gray-400 inline-block">
            Priority: <strong className="text-gray-700 dark:text-gray-300">{request.priorityReason?.split(';')[0]?.slice(0, 40)}</strong>
          </span>
          {request.riskReason && request.riskReason !== 'No suspicious patterns detected' && (
            <p className="text-[11px] text-gray-400 mt-1">{request.riskReason?.split(';')[0]?.slice(0, 60)}</p>
          )}
        </div>
        ...
        <div className="flex items-center justify-between gap-2 flex-wrap">
          ...
          <div className="flex gap-2">
            {user && user.role === 'donor' && !isOwner && (
              ...
              >
                {responded ? <span className="flex items-center gap-1"><CheckCircle2 className="w-3 h-3" /> Responded</span> : 'Respond'}
              </button>
            )}
            ...
            {!isOwner && (
              ...
              >
                {reported ? 'Reported' : <span className="flex items-center gap-1"><Flag className="w-3 h-3" /> Report</span>}
              </button>
            )}
          </div>
        </div>
        ...
        {request.respondedDonors?.length > 0 && (
          <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-800">
            <p className="text-xs text-emerald-600 dark:text-emerald-400 font-medium flex items-center gap-1">
              <CheckCircle2 className="w-3 h-3" /> {request.respondedDonors.length} donor(s) responded
            </p>
          </div>
        )}
      </div>
...
fs.writeFileSync('src/components/RequestCard.jsx', code, 'utf8')
console.log('RequestCard.jsx written successfully')
